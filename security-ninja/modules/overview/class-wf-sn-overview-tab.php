    ...
    <div class="sn-overview-container">
      ...
      <div class="testsandvulns">
        ...
        if ( $scores['good'] > 0 || $scores['bad'] > 0 || $scores['warning'] > 0 || $scores['score'] > 0 ) {
            echo '<div class="score-container">';
            echo '<h3>' . esc_html__( 'Your Security Test Results', 'security-ninja' ) . '</h3>';
            echo '<p>' . esc_html__( 'Here is a quick overview of how your site is doing:', 'security-ninja' ) . '</p>';
            echo '<ul style="list-style-type: none; padding: 0;">';
            echo '<li class="score-item"><strong>' . esc_html( $scores['good'] ) . '</strong> ' . esc_html__( 'tests passed successfully.', 'security-ninja' ) . '</li>';
            echo '<li class="score-item"><strong>' . esc_html( $scores['warning'] ) . '</strong> ' . esc_html__( 'tests have warnings.', 'security-ninja' ) . '</li>';
            echo '<li class="score-item"><strong>' . esc_html( $scores['bad'] ) . '</strong> ' . esc_html__( 'tests failed.', 'security-ninja' ) . '</li>';
            echo '<li class="score-item"><strong>' . esc_html( $scores['score'] ) . '%</strong> ' . esc_html__( 'is your overall security score.', 'security-ninja' ) . '</li>';
            echo '</ul>';
            echo '<p>Visit the <a href="#sn_tests">Security Tests</a> for more details.';
            echo '</div>';
        }
        ?>
        </div>
        ...
        if ( wf_sn_vu::$options['enable_vulns'] ) {
            ...
            if ( isset( $vuln_results['wordpress'] ) || isset( $vuln_results['plugins'] ) || isset( $vuln_results['themes'] ) ) {
                ...
                if ( $combined && count( $combined ) > 0 ) {
                    ...
                    echo '<div class="vulnerability-container">';
                    echo '<h3>' . esc_html__( 'Vulnerability Scan Results', 'security-ninja' ) . '</h3>';
            ...
            if ( empty( $combined ) ) {
                echo '<div class="vulnerability-container">';
                echo '<h3>' . esc_html__( 'Vulnerability Scan Results', 'security-ninja' ) . '</h3>';
                echo '<p>' . esc_html__( 'Great, no vulnerabilities found.', 'security-ninja' ) . '</p>';
                echo '</div>';
            }
            ?>
          </div>
      ...
      <?php 
            ...
            echo "Not tracking vulnerabilities.";
        }
        ?>
      </div><!-- .testanvulndshere -->
      <?php 
        // Check for plugin and theme updates
        $plugin_updates = get_plugin_updates();
        // Force refresh of theme updates using WordPress core functions
        ...
        if ( $total_updates > 0 ) {
            echo '<div class="sninnerrow">';
            echo '<h3>' . esc_html__( 'Available Updates', 'security-ninja' ) . '</h3>';
            // Build summary text with proper plural forms
            $summary = sprintf( _n(
                '%d update available',
                '%d updates available',
                $total_updates,
            ...
            echo '<p class="sn-updates-summary">' . esc_html( $summary ) . '</p>';
            echo '<p><a href="' . esc_url( admin_url( 'update-core.php' ) ) . '" class="">' . esc_html__( 'Go to WordPress Updates', 'security-ninja' ) . '</a></p>';
            echo '</div>';
        }
        ?>
      ...
      <?php 
        ...
        if ( $show_pro_ad ) {
            ?>
    <div class="sninnerrow upgradepro">
      <h3><?php 
      ...
      <a href="<?php 
            ...
            ?>" class="" target="_blank">30 days FREE trial &raquo;</a></small></p>
